...
public class Chassis extends SwerveChassis {
	/**
	 * Drive reduction, shared by the real module config and the simulated
	 * drivetrain below.
	 */
	 ...
	 *
	 * <p>
	 * The C++ declared four separate SimpleMotorFeedforward objects and then handed
	 * them to the
	 * wrong modules - FrontRightConfig() took feedForwardFrontLeft,
	 * BackRightConfig() took
	 * feedForwardFrontRight, and so on. All four held identical values so nothing
	 * misbehaved, but the
	 * first person to tune one module would have tuned a different wheel. One
	 * factory removes the
	 * possibility.
	 *
	 ...
	 *
	 * <p>
	 * These numbers are the ones RobotSim-Maple used for Shelby2, verified against
	 * overture/sim/swerve/Constants.java: 61.235 kg, a Pigeon2, 30.5 x 31.5 inch
	 * bumpers, and the
	 * same module translations as the kinematics above.
	 *
	 * <p>
	 * The gearing constants are the same ones the module configs use below. That is
	 * the entire
	 * point: the previous arrangement kept them in a separate simulation
	 * repository, where a drive
